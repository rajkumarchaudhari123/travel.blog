      ...
      <main className="relative w-full h-screen overflow-hidden">
        <div className="absolute inset-0 flex transition-transform duration-700 ease-in-out" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
          {images.map((image, index) => (
            <div key={index} className="relative min-w-full h-screen" data-aos="fade-in">
              <Image
                src={image.src}
                alt={image.alt}
                fill
        ...
        {/* Navigation Buttons */}
        <button onClick={prevSlide} className="absolute left-5 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 p-3 rounded-full text-white hover:bg-opacity-70 transition">
          ❮
        </button>
        <button onClick={nextSlide} className="absolute right-5 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 p-3 rounded-full text-white hover:bg-opacity-70 transition">
          ❯
        </button>
      ...
      <div className="flex flex-col items-center bg-gray-100 p-6 sm:p-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            { icon: "🚕", title: "City Transfer", description: "Fast and reliable city transfer services." },
